...
public final class Router {
    ...
    /// Pushes module, but removes all intermediate modules, when tap back -> shows first module
    public func pushAndRemoveIntermediate(_ module: UIViewController, animated: Bool) {
        guard let root = rootController?.viewControllers.first else { return }
        rootController?.hideBackTextAndSet(controllers: [root, module], animated: animated)
    }
